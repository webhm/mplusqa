...
class PacientesUCI extends App {
    ...
    static parseSeccionComburTest_v2(options) {
        ...
        options.map((option) => {
            FecthUci.dataSecciones.filter((obj) => {
                let _obj = JSON.parse(obj.DATASECCION);
                if (_obj.id === option.id && _obj.tipoBit == 'UCIINTER') {
                    res.push(_obj);
                }
            });
        });
                                    ...
                                    m("input[type='text'][placeholder='HH:mm']", {
                                        ...
                                        onkeypress: (e) => {
                                            if (e.keyCode == 13) {
                                                ...
                                                ComburTestUci.nuevoRegistro.timestamp = moment(PacientesUCI.fechaHoraTurno, 'DD-MM-YYYY HH:mm').format('DD-MM-YYYY') + ' ' + ComburTestUci.nuevoRegistro.hora;
